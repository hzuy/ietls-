      ...
      <AdminLayout>
        <div className="p-6 max-w-5xl">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h1 className="text-xl font-bold text-slate-800">{editing ? 'Chỉnh sửa Speaking Sample' : 'Thêm Speaking Sample mới'}</h1>
          </div>
            ...
            <div className="flex flex-col gap-4">
              <div className="bg-white rounded-xl p-5 border border-slate-200 shadow-sm">
                <div className="mb-3.5">
                  <label className="block text-xs font-semibold text-slate-600 mb-1.5">Tên chủ đề <span className="text-red-500 font-normal">*</span></label>
                ...
                <div className="mb-3.5">
                  <label className="block text-xs font-semibold text-slate-600 mb-1.5">Part</label>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-2">
                    {form.tags.map(t => (
                      <span key={t} className="inline-flex items-center gap-1 bg-purple-50 text-purple-700 rounded-full px-2.5 py-0.5 text-xs font-medium">
                        {t}<button onClick={() => setForm(f => ({ ...f, tags: f.tags.filter(x => x !== t) }))} aria-label={`Xóa tag ${t}`} className="bg-transparent border-0 cursor-pointer text-purple-700 text-sm leading-none">×</button>
                      </span>
                    ))}
                  </div>
                  <div className="flex gap-1.5">
                    ...
                    <button onClick={addTag} className="px-3 py-2 rounded-lg bg-purple-50 text-purple-600 text-sm font-semibold hover:bg-purple-100">+ Thêm</button>
                  </div>
                </div>
              </div>
      ...
      {delConfirm && (
        <div onClick={() => setDelConfirm(null)} style={{ position: 'fixed', inset: 0, zIndex: 9999, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
          <div onClick={e => e.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby="del-confirm-title" className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-sm">
            <div className="flex items-center gap-3 mb-3"><div className="w-10 h-10 rounded-full bg-rose-100 flex items-center justify-center text-xl">🗑️</div><h3 id="del-confirm-title" className="font-bold text-gray-800">Xóa bài Speaking?</h3></div>
