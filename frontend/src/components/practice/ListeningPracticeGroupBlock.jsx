            ...
            return (
              <span key={i} className="inline-flex items-center gap-1 mx-1">
                ...
                <select value={val} onChange={e => onAnswer(qNum, e.target.value)}
                  className="border border-zinc-300 rounded-full bg-white outline-none text-sm px-3 py-0.5 text-zinc-900 max-w-36 focus:border-zinc-900">
                  <option value="">—</option>
    ...
    <div id={`q-${group.qNumberStart}`} className="mb-6 scroll-mt-4">
      ...
      {group.type === 'note_completion' && (group.noteSections || []).map((ns, nsi) => (
        <div key={nsi} className="mb-4">
          ...
          <div className="bg-white border border-zinc-200 rounded-2xl p-5 space-y-1 shadow-xs">
      ...
      {group.type === 'matching' && (
        <div>
          <div className="flex flex-wrap gap-1.5 mb-4">
            {opts.map((mo, mi) => (
              <span key={mi} className="text-xs px-3 py-1 bg-zinc-100 border border-zinc-200 text-zinc-800 rounded-full">
          ...
          {(group.questions || []).map(q => (
            <div key={q.number} id={`q-${q.number}`} className="flex items-center gap-3 mb-3 scroll-mt-4">
              ...
              <select value={answers[q.number] || ''} onChange={e => onAnswer(q.number, e.target.value)}
                className="border border-zinc-300 rounded-full px-3.5 py-1.5 text-sm outline-none focus:border-zinc-900 text-zinc-900 bg-white min-w-32">
        ...
        <div>
          {group.imageUrl && (
            <img src={toImgSrc(group.imageUrl)}
              alt="diagram" className="w-full max-w-sm rounded-2xl border border-zinc-200 mb-4 object-contain bg-zinc-50" />
          )}
          <div className="flex flex-wrap gap-1.5 mb-4">
            {opts.map((mo, mi) => (
              <span key={mi} className="text-xs px-3 py-1 bg-zinc-100 border border-zinc-200 text-zinc-800 rounded-full">
      ...
      {group.type === 'drag_word_bank' && (
        <div>
          <div className="flex flex-wrap gap-2 mb-4 p-5 bg-white border border-zinc-200 rounded-2xl shadow-xs">
            <p className="w-full text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-1">Word Bank</p>
            {opts.map((mo, mi) => (
              <span key={mi} className="text-xs px-3.5 py-1.5 bg-zinc-50 border border-zinc-200 rounded-full font-medium text-zinc-800">
      ...
      {group.type === 'matching_drag' && (
        <div className="flex gap-4">
          <div className="flex-1 space-y-3">
            {(group.questions || []).map(q => (
              <div key={q.number} id={`q-${q.number}`} className="bg-white rounded-2xl border border-zinc-200 p-4 shadow-xs scroll-mt-4">
                <p className="text-sm text-zinc-900 mb-2 flex gap-2">
                  <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-zinc-100 border border-zinc-200 text-zinc-800 font-bold text-xs shrink-0 mt-0.5">{q.number}</span>
                  <span>{q.questionText}</span>
                </p>
                <select value={answers[q.number] || ''} onChange={e => onAnswer(q.number, e.target.value)}
                  className="w-full border border-zinc-300 rounded-full px-3.5 py-1.5 text-sm outline-none focus:border-zinc-900 text-zinc-900 bg-white">
          ...
          <div className="w-44 shrink-0">
            ...
            <div className="space-y-1.5">
              {opts.map((mo, oi) => (
                <div key={oi} className="flex items-start gap-2 px-3.5 py-2 rounded-full border border-zinc-200 bg-white text-xs">
