...
export default function TableCompletionEditor({
  group = {},
  onChange,
  numberingMode = 'auto',
  themeSource = 'admin'
  ...
  const groupType = group?.type || 'table_completion'
  ...
  const rebuildSection = (newHeaderCells, newDataLines) => {
    ...
    onChange({
      ...
      ...syncQuestionsToTokens(newLines.map(l => l.content), group.questions || [], group.qNumberStart),
    })
  }
  ...
  const updateTitle = (val) => {
    onChange({
      ...group,
      noteSections: [{ ...section, title: val }, ...(group.noteSections || []).slice(1)]
    })
  }
      ...
      <div className={`${theme.subBoxBg} border ${theme.subBoxBorder} rounded-lg p-4`}>
        <div className="flex items-center gap-3 mb-3">
          ...
          <div className="flex items-center gap-2 ml-auto">
            <span className="text-xs text-zinc-500">Số cột:</span>
            <button
              type="button"
              onClick={() => colCount > 2 && setColCount(colCount - 1)}
              disabled={colCount <= 2}
              className="w-6 h-6 flex items-center justify-center rounded border border-zinc-200 text-zinc-500 hover:bg-zinc-100 disabled:opacity-30 text-xs font-bold transition"
            >
              −
            </button>
            <span className="w-5 text-center text-sm font-bold text-zinc-700">{colCount}</span>
            <button
              type="button"
              onClick={() => colCount < 6 && setColCount(colCount + 1)}
              disabled={colCount >= 6}
              className="w-6 h-6 flex items-center justify-center rounded border border-zinc-200 text-zinc-500 hover:bg-zinc-100 disabled:opacity-30 text-xs font-bold transition"
            >
              +
              ...
              {dataLines.map((dl, ri) => {
                ...
                return (
                  <tr key={ri} className={ri % 2 === 0 ? 'bg-white' : 'bg-zinc-50'}>
                    {Array.from({ length: colCount }, (_, ci) => (
                      <td key={ci} className="px-2 py-1.5 border border-zinc-200 align-top min-w-[100px]">
                    ...
                    <td className="border border-zinc-200 px-1 text-center align-middle">
                      {dataLines.length > 1 && (
                        <button
                          type="button"
        ...
        <button
          type="button"
          onClick={addRow}
          className={`mt-2 text-xs ${theme.subBoxText} font-semibold hover:underline`}
        >
          + Thêm hàng
        </button>
      </div>
        ...
        <div className={`${theme.subBoxBg} border ${theme.subBoxBorder} rounded-lg p-3`}>
          <div className="flex items-center justify-between mb-2">
            ...
            <p className="text-[10px] text-zinc-400">
              Dùng <span className="font-mono bg-zinc-100 px-1 rounded">/</span> để tách nhiều đáp án. VD: <span className="font-mono">word1/word2</span>
            </p>
          </div>
            ...
            {tokenOrder.map(tokenNum => {
              ...
              return (
                <div key={tokenNum} className="flex items-center gap-2">
                  ...
                  <input
                    className={`flex-1 border ${theme.subBoxBorder} bg-white rounded-lg px-2 py-1 text-sm focus:outline-none`}
