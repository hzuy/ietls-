 ...
 * Bản kia: src/components/practice/NoteCompletionEditor.jsx
      ...
      <div className={`${theme.subBoxBg} border ${theme.subBoxBorder} rounded-lg p-4`}>
        <div className="flex items-center justify-between mb-3">
          ...
          <button type="button" onClick={addSection}
            className={`text-xs ${theme.subBoxText} font-semibold hover:underline`}>+ Thêm phần</button>
        </div>
              ...
              <div className="flex items-center gap-2 mb-2">
                ...
                {group.noteSections.length > 1 && (
                  <button type="button" onClick={() => removeSection(nsi)}
                    className="text-red-500 hover:text-red-600 text-xs px-2">✕</button>
                )}
              </div>
                    ...
                    <div key={li} className={`flex items-start gap-2 rounded-lg p-1 ${isHeading ? 'bg-slate-50' : ''}`}>
                      ...
                      <textarea
                        ...
                        className={`flex-1 border rounded-lg px-2 py-1 text-sm resize-none focus:outline-none font-mono ${
                          isHeading
                            ? 'font-bold border-slate-300 focus:border-slate-400 bg-slate-50 text-slate-700'
                      ...
                      {ns.lines.length > 1 && (
                        <button type="button" onClick={() => removeLine(nsi, li)}
                          className="shrink-0 text-red-500 hover:text-red-600 text-xs py-1.5">✕</button>
                      )}
                    </div>
                  )
                })}
                <button type="button" onClick={() => addLine(nsi)}
                  className="text-xs text-slate-400 hover:text-slate-600 font-medium">+ Thêm dòng</button>
              </div>
            </div>
          ))}
        </div>
      ...
      {tokenOrder.length > 0 && (
        <div className={`${theme.subBoxBg} border ${theme.subBoxBorder} rounded-lg p-3`}>
          <div className="flex items-center justify-between mb-2">
            ...
            <p className="text-[10px] text-slate-400">Dùng <span className="font-mono bg-slate-100 px-1 rounded">/</span> để tách nhiều đáp án chấp nhận được. VD: <span className="font-mono">intestine/gut</span></p>
          </div>
