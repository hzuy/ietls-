      ...
      {['fill_blank', 'diagram_completion'].includes(q.type) && (
        <div className="pl-8">
          ...
          <input type="text"
            ...
            className={`border-b-2 ${previewMode && showAnswers ? 'border-green-500 text-green-700 font-semibold' : 'border-zinc-300 focus:border-zinc-900 text-zinc-900'} outline-none px-2 py-1 text-sm w-64 bg-transparent transition`} />
        </div>
      )}
        ...
        <div className="pl-8">
          ...
          {opts.length > 0 ? (
            ...
            <input type="text"
              ...
              onChange={previewMode ? undefined : e => onAnswer(q.id, e.target.value)}
              placeholder={previewMode ? '' : 'Nhập đáp án...'}
              className={`border-b-2 ${previewMode && showAnswers ? 'border-green-500 text-green-700 font-semibold' : 'border-zinc-300 focus:border-zinc-900 text-zinc-900'} outline-none px-2 py-1 text-sm w-48 bg-transparent transition`} />
          )}
        </div>
      )}
    </div>
